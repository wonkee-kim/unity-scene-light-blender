...
Shader "Skybox/Panoramic-Blend" {
    Properties {
        ...
        [Toggle(_USE_BLENDER)] _USE_BLENDER ("Use Blender", Float) = 0
        // [Enum(_TEX1, 0, _TEX2, 1)] _Selector ("Selector", Float) = 0
        [KeywordEnum(TEX1, TEX2)] _Selector ("Selector", Float) = 0
        _BlendSlider ("Blend", Range(0, 1)) = 0.5
    ...
    SubShader {
        ...
        Pass {
            ...
            #pragma multi_compile_local _ _USE_BLENDER
            #pragma multi_compile_local _SELECTOR_TEX1 _SELECTOR_TEX2
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                    ...
                    float2 tc2 = ToCubeCoords(i.texcoord2, i.layout, i.edgeSize, i.faceXCoordLayouts, i.faceYCoordLayouts, i.faceZCoordLayouts);
                #else
                    float2 tc = ToRadialCoords(i.texcoord);
                    float2 tc2 = ToRadialCoords(i.texcoord2);
                ...
                #if defined(_USE_BLENDER)
                    half4 tex = tex2D (_MainTex, tc);
                    half3 c1 = DecodeHDR (tex, _MainTex_HDR);
                    half4 tex2 = tex2D (_MainTex2, tc2);
                    half3 c2 = DecodeHDR (tex2, _MainTex2_HDR);
                    float t = _BlendSlider;
                    // t = t * t * (3.0 - 2.0 * t); // smoothstep
                    // t = t * t * t * (t * (t * 6 - 15) + 10); // smootherstep
                    // t = t * t; // bias to night
                    half3 c = lerp(c1, c2, t);
                #else
                    #if defined(_SELECTOR_TEX1)
                        half4 tex = tex2D (_MainTex, tc);
                        half3 c = DecodeHDR (tex, _MainTex_HDR);
                    #else
                        half4 tex2 = tex2D (_MainTex2, tc2);
                        half3 c = DecodeHDR (tex2, _MainTex2_HDR);
                    #endif
                #endif
